...
static uint8_t wwwport_l = 0; // server port
...
void client_browse_url(char *urlbuf, char *urlbuf_varpart, char *hoststr, void (*callback)(uint8_t,uint16_t,uint16_t))
{
        ...
        www_fd=client_tcp_req(&www_client_internal_result_callback,&www_client_internal_datafill_callback,wwwport_l);
}
...
uint16_t packetloop_icmp_tcp(uint8_t *buf, uint16_t plen) {
	...
	if (plen == 0) {
		...
		if (tcp_client_state == 1 && (waitgwmac & WGW_HAVE_GW_MAC)) { // send a syn
			...
			// from the server:
			client_syn(buf, ((tcp_fd << 5) | (0x1f & tcpclient_src_port_l)),
	...
	if (buf[IP_PROTO_P] == IP_PROTO_UDP_V && buf[UDP_SRC_PORT_H_P] == 0
